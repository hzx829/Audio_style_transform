...
from model_2D import RandomCNN


data_path = '/Volumes/胖胖/something/VCTK-Corpus/'
...
def process_vctk(_data_path, speaker_num=2, each_audio_num=15):
    # read label-info
    ...
    for d in [_data_path + 'txt/p%d/' % uid for uid in df.ID.values[:speaker_num]]:
        file_ids.extend([f[-12:-4] for f in sorted(glob.glob(d + '*.txt')[:each_audio_num])])#-12 is the beginning of txt, -4 is for .txt

    audio_lst = []
    for i, f in enumerate(file_ids):
        # wave file name
        wave_file = _data_path + 'wav48/%s/' % f[:4] + f + '.wav'#4 is for specific files
        fn = wave_file.split('/')[-1].split("_")[0]#fn is the name of specific files ,and it represents id of speaker
        print(fn)
...
def spect2gram(spect_lst):
    grams_lst = []
    counter_step = 0
    list_len = len(spect_lst)
    for item in spect_lst:
        print("gram computing... Processing(%d/%d)"%(counter_step,list_len))
...
counter_test_step = 0
len_test = len(test_grams)
for item in test_grams:
    print("Processing (%d/%d)" %(counter_test_step,len_test))
